...
class HouseModel:
    ...
    def __str__(self):
        return f"编号{self.id},标题{self.title},的小区{self.community}年份是{self.years}房型为{self.house_type}" \
               f",建筑面积是{self.area};底层为{self.floor}描述信息:{self.description};总价为{self.total_price}" \
               f",单价是{self.unit_price};带看记录:{self.follow_info}"
